...
#
#  Created by Mark D. Preston, LSHTM 2014, see www.markdpreston.com.
#
#  Modifed by Thomas Abeel
#
#use strict;
...
my $lsPath = '/gsap/garage-bacterial/Projects/TB-Global/colldr/';
...
open T, ">$lsPath/output/snap.$psId.tsv";
...
outputLog("Mapping");
`$lsSnap single $lsPath/process $laFastq[0] -o $lsBam -so -sm 2 -t 2 -M 2> $lsError`;
...
foreach my $lsDrug (@laDrugOrder) {
  if (exists($lhDr{$lsDrug})) {
	...
	outputTab(ucfirst(lc($lsDrug)).+"\tR\t".$lhDr{$lsDrug}."\n");
  } else {
    outputHtml("<tr><td>" . ucfirst(lc($lsDrug)) . "</td><td>&nbsp;</td><td>&nbsp;</td></tr>\n");
	outputTab(ucfirst(lc($lsDrug)).+"\tS\t-\n");
  }
}
outputHtml("</table>\n");
